...
app.action('button_click', handleButton);
...
async function requestBirthday () {
  ...
  try {
    ...
    console.log('=== CHANNEL RESULT VARIABLE ===', channelResult);
...
app.shortcut('save4later', async ({ shortcut, ack, say, respond, context }) => {
  ack()
...
app.command('/new', async ({ command, ack, payload, context, respond }) => {
...
app.view('survey', async ({ ack, body, view, context }) => {
  ...
  console.log('=== RAW USER RESPONSES ===', userResponsesRaw);

  const userResponses = {
    enjoy_working: userResponsesRaw.enjoy_working.enjoy_working_action.selected_option.value,
    lunch: userResponsesRaw.lunch.lunch_action.selected_options.map((c) => {  console.log('+++ C VALUE +++', c); return c.text.text }),
